...
const Todos = () => {
    const Todos = useSelector((ele) => ele.todos);
            ...
            <ul className="list-none">
                {Todos.map((ele) => {
                    return (
                        <li
                            key={ele.id}
                            className="mt-4 w-fit flex justify-between items-center bg-zinc-800 px-4 py-2 rounded">
                            <div className="text-white">{ele.text}</div>
                            <button
                                onClick={() => handleRemove(ele.id)}
                                className="text-white bg-red-500 border-0 py-1 px-2 ml-4 focus:outline-none hover:bg-red-600 rounded text-md">
                                <svg
